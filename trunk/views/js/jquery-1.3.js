...
( function() {
	...
	jQuery.fn = jQuery.prototype = {
		...
		find : function(selector) {
			if (this.length === 1 && !/,/.test(selector)) {
				...
				return this.pushStack(/[^+>] [^+>]/.test(selector) ? jQuery
						.unique(elems) : elems, "find", selector);
			}
		},
		...
		domManip : function(args, table, callback) {
			if (this[0]) {
				var fragment = (this[0].ownerDocument || this[0])
						.createDocumentFragment(), scripts = jQuery.clean(args,
						(this[0].ownerDocument || this[0]), fragment), first = fragment.firstChild, extra = this.length > 1 ? fragment
						.cloneNode(true)
						: fragment;

				if (first)
					for ( var i = 0, l = this.length; i < l; i++)
						callback.call(root(this[i], first), i > 0 ? extra
...
empty : function() {
	...
	( function() {
		...
		var Sizzle = function(selector, context, results, seed) {
			...
			if (parts.length > 1 && Expr.match.POS.exec(selector)) {
				if (parts.length === 2 && Expr.relative[parts[0]]) {
					...
					set = Sizzle.filter(later, Sizzle(
							/\s$/.test(selector) ? selector + "*" : selector,
							context));
				} else {
						...
						if (match) {
							for ( var i = 0; (item = curLoop[i]) !== undefined; i++) {
								if (item) {
									...
									found = filter(item, match, goodPos,
											goodArray);
									var pass = not ^ !!found;
		...
		var Expr = Sizzle.selectors = {
			...
			relative : {
				"+" : function(checkSet, part) {
					for ( var i = 0, l = checkSet.length; i < l; i++) {
						...
						if (elem) {
							...
							checkSet[i] = typeof part === "string" ? cur || false
									: cur === part;
						}
					}
				...
				">" : function(checkSet, part, isXML) {
					if (typeof part === "string" && !/\W/.test(part)) {
						...
						for ( var i = 0, l = checkSet.length; i < l; i++) {
							var elem = checkSet[i];
							if (elem) {
								checkSet[i] = typeof part === "string" ? elem.parentNode
				...
				"" : function(checkSet, part, isXML) {
					...
					if (!part.match(/\W/)) {
						var nodeCheck = part = isXML ? part : part
								.toUpperCase();
			...
			find : {
				...
				NAME : function(match, context) {
					return context.getElementsByName ? context
							.getElementsByName(match[1]) : null;
				},
			...
			preFilter : {
				...
				TAG : function(match, curLoop) {
					...
					return isXML(curLoop[i]) ? match[1] : match[1]
							.toUpperCase();
				},
			...
			filter : {
				...
				ATTR : function(elem, match) {
					var result = Expr.attrHandle[match[1]] ? Expr.attrHandle[match[1]]
							(elem)
							: elem[match[1]] || elem.getAttribute(match[1]), value = result
		...
		( function() {
			...
			var form = document.createElement("form"), id = "script"
					+ (new Date).getTime();
			...
			if (!!document.getElementById(id)) {
				...
				Expr.filter.ID = function(elem, match) {
					var node = elem.getAttributeNode
								...
								// Namespaced event handlers
									...
									// remove all handlers for the given type
										...
										for ( var handle in events[type])
											...
											// events
											if (namespace
													.test(events[type][handle].type))
												delete events[type][handle];

									if (jQuery.event.specialAll[type])
										jQuery.event.specialAll[type].teardown
	...
	var elemdisplay = {}, fxAttrs = [ // height animations
			[ "height", "marginTop", "marginBottom", "paddingTop",
					"paddingBottom" ], // width animations
			[ "width", "marginLeft", "marginRight", "paddingLeft",
					"paddingRight" ], // opacity animations
			[ "opacity" ] ];
					...
					if (speed) {
						...
						for ( var i = 0, l = this.length; i < l; i++) {
							...
							if (jQuery.css(this[i], "display") === "none") {
								...
								this[i].style.display = jQuery.data(this[i],
										"olddisplay", display);
							}
						}
				...
				hide : function(speed, callback) {
					if (speed) {
						...
						for ( var i = 0, l = this.length; i < l; i++) {
							var old = jQuery.data(this[i], "olddisplay");
							if (!old && old !== "none")
								jQuery.data(this[i], "olddisplay", jQuery.css(
										this[i], "display"));
				...
				animate : function(prop, speed, easing, callback) {
					...
					return this[optall.queue === false ? "each" : "queue"]
							( function() {
								...
								var opt = jQuery.extend( {}, optall), p, hidden = this.nodeType == 1
	...
	if (document.documentElement["getBoundingClientRect"])
		jQuery.fn.offset = function() {
			...
			var box = this[0].getBoundingClientRect(), doc = this[0].ownerDocument, body = doc.body, docElem = doc.documentElement, clientTop = docElem.clientTop
					...
					+ (self.pageYOffset || jQuery.boxModel && docElem.scrollTop || body.scrollTop)
					- clientTop, left = box.left
					+ (self.pageXOffset || jQuery.boxModel
			...
			while ((elem = elem.parentNode) && elem !== body
				...
				if (elem === offsetParent) {
					...
					if (jQuery.offset.doesNotAddBorder
							&& !(jQuery.offset.doesAddBorderForTableAndCells && /^t(able|d|h)$/i
									.test(elem.tagName)))
	...
	jQuery.offset = {
		initialize : function() {
			...
			innerDiv = container.firstChild, checkDiv = innerDiv.firstChild,
		...
		bodyOffset : function(body) {
			...
			if (jQuery.offset.doesNotIncludeMarginInBodyOffset)
						top += parseInt(jQuery.curCSS(body, 'marginTop', true),
								10) || 0, left += parseInt(jQuery.curCSS(body,
	...
	jQuery.fn.extend( {
		position : function() {
			...
			if (this[0]) {
				...
				offset = this.offset(), parentOffset = /^body|html$/i
